use crate::{
    components::{
        dropdown_menu::{
            DropdownAlign, DropdownMenu, DropdownMenuContent, DropdownMenuItem,
            DropdownMenuSeparator, DropdownMenuTrigger, DropdownSide,
        },
        select::{
            Select, SelectAlign, SelectItemIndicator, SelectList, SelectOptionItem, SelectSide,
...
pub fn BottomBar() -> Element {
    ...
    // Watch for context changes to sync local state
    let state_for_watch = state.clone();
    use_future(move || {
        let state_for_watch = state_for_watch.clone();
        let mut selected_org_id = selected_org_id.clone();
        let mut selected_project_id = selected_project_id.clone();
        let mut pending_org_switch = pending_org_switch.clone();
        async move {
    ...
    rsx! {
        div { class: "shrink-0 flex items-center justify-between gap-2 px-2 h-12 border-t border-app-border bg-background",
            div { class: "relative mr-auto",
                DropdownMenu {
                    ...
                    DropdownMenuTrigger { class: "flex items-center gap-1 justify-start focus:outline-2 focus:outline-app-border/50",
                        div { class: "w-7 h-7 rounded-md border border-app-border bg-white flex items-center justify-center cursor-default",
                        ...
                        div { class: "flex items-center gap-1 cursor-default",
                            span { class: "text-xs text-foreground truncate max-w-[200px]",
                                "{user_email}"
                            }
                            Icon {
                                source: IconSource::Named("chevron-down".into()),
                                size: 10,
                                class: "text-icon-select mb-0.5",
                            }
                        }
                    }
                    DropdownMenuContent {
            ...
            // Left side: org/project selectors
            if state.selected_context().is_some() || !orgs_snapshot.is_empty() {
...
pub fn Sidebar() -> Element {
    ...
    let sidebar_class = if sidebar_hidden {
        "min-w-[190px] max-w-[190px] shrink-0 flex-none bg-background border-r border-app-border pt-5 pb-6 px-4 flex flex-col absolute left-0 -translate-x-[190px] z-10"
    } else {
        "min-w-[190px] max-w-[190px] shrink-0 flex-none bg-background border-r border-app-border pt-5 pb-6 px-4 flex flex-col"
    };
    ...
    rsx! {
        // Content row
        div { class: "flex flex-1 min-h-0 relative flex-col",
